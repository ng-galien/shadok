...
public class PodMutatingWebhook {
    ...
    Function<Pod, Optional<CrdRef>> findAnnotation = pod ->
    ...
    Function<CrdRef, Optional<ApplicationSpec>> findAppSpec(KubernetesClient kubernetesClient) {
        return ref->
            Optional.ofNullable(kubernetesClient.
    ...
    Function<ApplicationSpec, UnaryOperator<Pod>> mutateOp = appSpec -> pod -> {
    ...
    public sealed interface PodMutation permits 
    ...
    private MutationContext createMutationContext(ApplicationSpec appSpec, Pod pod) {
        var projectSource = findProjectSource(appSpec.projectSourceName(), pod.getMetadata().getNamespace());
        var dependencyCache = findDependencyCache(appSpec.dependencyCacheName(), pod.getMetadata().getNamespace());
        var applicationType = appSpec.applicationType();
        
        var mutations = Stream.of(
    ...
    private Pod applyMutations(Pod pod, MutationContext context) {
        return context.mutations().stream()
            .reduce(pod, this::applyMutation, (p1, p2) -> p2);
    }
    ...
    private Pod applyMutation(Pod pod, PodMutation mutation) {
        return switch (mutation) {
            ...
            case PodMutation.AddVolumeMount(var containerName, var mount) -> addVolumeMount(pod, containerName, mount);
            case PodMutation.AddInitContainer(var initContainer) -> addInitContainer(pod, initContainer);
            case PodMutation.TransformMainContainer(var transformation) -> transformMainContainer(pod, transformation);
        };
    }

    // Functions for creating mutations
    private List<PodMutation> createVolumeMutations(Optional<ProjectSource> projectSource, Optional<DependencyCache> dependencyCache) {
        return Stream.of(
    ...
    private List<PodMutation> createInitContainerMutations(ApplicationSpec appSpec, Optional<ProjectSource> projectSource) {
    ...
    private Optional<PodMutation> createInitContainerMutation(InitContainerMountSpec mountSpec, Optional<ProjectSource> projectSource) {
        return projectSource.map(ps -> {
    ...
    private List<PodMutation> createMainContainerMutations(ApplicationType applicationType) {
        return List.of(
            new PodMutation.TransformMainContainer(container -> transformForLiveReload(container, applicationType)),
    ...
    private Pod addVolume(Pod pod, Volume volume) {
        var volumes = new ArrayList<>(Optional.ofNullable(pod.getSpec().getVolumes()).orElse(List.of()));
        volumes.add(volume);
    ...
    private Pod addVolumeMount(Pod pod, String containerName, VolumeMount mount) {
        pod.getSpec().getContainers().stream()
            ...
            .findFirst()
            .ifPresent(container -> {
                var mounts = new ArrayList<>(Optional.ofNullable(container.getVolumeMounts()).orElse(List.of()));
                mounts.add(mount);
    ...
    private Pod addInitContainer(Pod pod, Container initContainer) {
        var initContainers = new ArrayList<>(Optional.ofNullable(pod.getSpec().getInitContainers()).orElse(List.of()));
    ...
    private Pod transformMainContainer(Pod pod, UnaryOperator<Container> transformation) {
        pod.getSpec().getContainers().stream()
            ...
            .map(transformation)
            .ifPresent(transformedContainer -> pod.getSpec().getContainers().set(0, transformedContainer));
        return pod;
    }
    ...
    record LiveReloadConfig(List<String> command, List<EnvVar> envVars, List<ContainerPort> debugPorts) {}

    private LiveReloadConfig getLiveReloadConfig(ApplicationType applicationType) {
        return switch (applicationType) {
            case SPRING -> new LiveReloadConfig(
                List.of("mvn", "spring-boot:run"),
                List.of(
                    new EnvVarBuilder().withName("MAVEN_OPTS").withValue("-Dmaven.repo.local=/cache/.m2/repository").build(),
                    new EnvVarBuilder().withName("JAVA_TOOL_OPTIONS").withValue("-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:5005").build()
                ),
                List.of(new ContainerPortBuilder().withContainerPort(5005).withName("debug").build())
            );
            case QUARKUS -> new LiveReloadConfig(
            ...
            case NODE -> new LiveReloadConfig(
                List.of("npm", "run", "dev"),
                List.of(new EnvVarBuilder().withName("NODE_ENV").withValue("development").build()),
                List.of(new ContainerPortBuilder().withContainerPort(9229).withName("debug").build())
            );
            case PYTHON -> new LiveReloadConfig(
                ...
                List.of(new ContainerPortBuilder().withContainerPort(5678).withName("debug").build())
            );
            case GO -> new LiveReloadConfig(
                List.of("go", "run", "main.go"),
                List.of(),
                List.of(new ContainerPortBuilder().withContainerPort(40000).withName("debug").build())
            );
            case RUBY -> new LiveReloadConfig(
                List.of("bundle", "exec", "rails", "server"),
                List.of(),
                List.of(new ContainerPortBuilder().withContainerPort(1234).withName("debug").build())
            );
            case PHP -> new LiveReloadConfig(
                List.of("php", "-S", "0.0.0.0:8080"),
                List.of(),
                List.of(new ContainerPortBuilder().withContainerPort(9003).withName("debug").build())
            );
            case DOTNET -> new LiveReloadConfig(
                List.of("dotnet", "watch", "run"),
                List.of(),
                List.of()
            );
