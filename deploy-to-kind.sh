...
build_operator_image() {
    ...
    # Aller dans le répertoire operator
    cd operator || {
        print_error "Répertoire operator/ non trouvé"
        exit 1
    }
...
deploy_crds() {
    ...
    # Appliquer les CRDs depuis le répertoire operator
    if [ -d "operator/kubernetes" ]; then
        kubectl apply -f operator/kubernetes/ || {
            print_error "Échec du déploiement des CRDs"
            exit 1
        }
    else
        print_error "Répertoire operator/kubernetes non trouvé"
        exit 1
    fi
...
if [ ! -f "settings.gradle.kts" ] || [ ! -d "operator" ]; then
    print_error "Ce script doit être exécuté depuis la racine du projet Shadok"
    print_error "Répertoires requis: operator/, pods/"
    exit 1
fi
