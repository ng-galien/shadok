...
import argparse, hashlib, json, os, pathlib, platform, subprocess, tarfile, tempfile, urllib.request
...
for name in (archive,f'SHA256SUMS-{a.version}'):
    with urllib.request.urlopen(base+name,timeout=120) as response:(work/name).write_bytes(response.read())
...
evidence={'version':a.version,'chart':chart,'cliSHA256':checks[archive],'images':images,'checks':['anonymous CLI and chart download','checksum verification','fresh Kind registry installation','HTTPS direct NodePort','CR-only identity','two-replica sync','add/delete','pod replacement','exact restoration','helm test']}
