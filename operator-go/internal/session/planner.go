...
func Plan(s *api.DevelopmentSession, d *appsv1.Deployment, image string) (*corev1.Pod, error) {
	...
	for i, v := range s.Spec.Directories {
		if !syncer.ValidMount(v.Name) || names[v.Name] || !validPath(v.ImagePath) || !validPath(v.MountPath) {
			return nil, fmt.Errorf("invalid directory %q", v.Name)
		}
		names[v.Name] = true
		if overlaps(v.ImagePath, "/shadok-tools") || overlaps(v.ImagePath, "/shadok-seed") {
