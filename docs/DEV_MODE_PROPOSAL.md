# Architecture decision: live mode on an existing Deployment
...
External template conflicts, per-file atomicity, periodic scans, missing distributed writer leases and separate application/HMR verification remain explicit constraints. Operational instructions are embedded in `shadok learn`; review and validation records live alongside this decision.
